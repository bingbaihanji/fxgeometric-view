...
public class LineConstraint implements PointConstraint {
    ...
    public void setAsVertexConstraintIfApplicable(com.bingbaihanji.view.layout.draw.geometry.impl.PointGeo point) {
        // 检查点是否是线段的端点
        com.bingbaihanji.view.layout.draw.geometry.impl.PointGeo startRef = line.getStartPointRef();
        com.bingbaihanji.view.layout.draw.geometry.impl.PointGeo endRef = line.getEndPointRef();
        ...
        if (startRef != null && startRef == point) {
            ...
            this.parameter = 0.0;
        } else if (endRef != null && endRef == point) {
            // 点就是终点引用
            this.isVertexConstraint = true;
            this.parameter = 1.0;
        } else {
            // 检查坐标是否非常接近端点
            double distToStart = MathCalculationUtils.hypot(
                    point.getX() - line.getStartX(),
                    point.getY() - line.getStartY()
            );
            double distToEnd = MathCalculationUtils.hypot(
                    point.getX() - line.getEndX(),
                    point.getY() - line.getEndY()
            );
